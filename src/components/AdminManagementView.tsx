...
export const AdminManagementView: React.FC<AdminManagementViewProps> = ({
  ...
  const handleUpdateRole = async () => {
    ...
    if (supabase) {
      try {
        await supabase.from('users').update({ role: newRole }).eq('id', selectedUser.id);
      } catch (e) {
        console.error(e);
      }
    }
  ...
  const handleAdminResetPassword = async () => {
    ...
    if (supabase) {
      try {
        await supabase.from('users').update({ password_hash: newHash }).eq('id', selectedUser.id);
      } catch (e) {
        console.error(e);
      }
    }
  ...
  const handleSaveViewPermissions = async () => {
    ...
    if (supabase) {
      try {
        await supabase.from('users').update({ allowed_views: selectedUserViews }).eq('id', selectedUser.id);
      } catch (e) {
